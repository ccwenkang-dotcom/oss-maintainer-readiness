...
def collect_public_github_evidence(repo_ref: str, timeout: float = 10.0) -> Evidence:
    ...
    tests_exist = _exists(f"{repo_url}/contents/tests", timeout)
    ci_exists = _exists(f"{repo_url}/contents/.github/workflows", timeout)
    contributing_exists = _exists_any(
        (
            f"{repo_url}/contents/CONTRIBUTING.md",
            f"{repo_url}/contents/.github/CONTRIBUTING.md",
        ),
        timeout,
    )
    security_policy_exists = _exists_any(
        (
            f"{repo_url}/contents/SECURITY.md",
            f"{repo_url}/contents/.github/SECURITY.md",
        ),
        timeout,
    )
...
def _exists_any(urls: tuple[str, ...], timeout: float) -> bool:
    return any(_exists(url, timeout) for url in urls)
